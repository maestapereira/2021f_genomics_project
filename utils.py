...
def get_c_g_features(df_feature, in_numpy=False):
    """Makes feature pd.DataFrame into numpy array with floats and ints only
    ...
    gene_feature : `np.ndarray`
        Gene expression features nd array
    """
    bio_cols = [col for col in df_feature.columns if (col.startswith('g-')|col.startswith('c-')|col.startswith('id'))]
    bio_feature = df_feature[bio_cols]
    if in_numpy == True:
        if np.isin('id', bio_feature.columns):
            bio_feature = bio_feature.drop(columns=['id'])
        if np.isin('dtype', bio_feature.columns):
            bio_feature = bio_feature.drop(columns=['dtype'])
        bio_feature = bio_feature.to_numpy()
...
def split_data_train_test_val(df, in_numpy=False):
    """Returns train, test and validation sets of features

    Parameters
    ----------
    df : `pd.DataFrame`
        pandas data frame with features and potentially also labels
    in_numpy : `bool`
        if output should be returned in numpy, with integers and floats only 
        (True) or in pd.DataFrame (False)
        Defaults to False.
    ...
    """
    temp = df
    if np.isin('id', df.columns):
        temp = temp.drop(columns=['id', 'dtype'])
    if np.isin('dtype', df.columns):
        temp = temp.drop(columns=['dtype'])
    train_features = temp[df.iloc[:, -1] == 'train'].reset_index(drop=True)
    test_features = temp[df.iloc[:, -1] == 'test'].reset_index(drop=True)
    val_features = temp[df.iloc[:, -1] == 'val'].reset_index(drop=True)
    if in_numpy==True:
        train_features = _features_from_pd_to_np(train_features)
        test_features = _features_from_pd_to_np(test_features)
        val_features = _features_from_pd_to_np(val_features)
